...
class ImageHash(size: Int = 32, smallerSize: Int = 8) {
  val c: Array[Double] = {
  ...
  def getHash(is: InputStream): String = {
    ...
    val vals: Array[Array[Double]] = Array.ofDim[Double](size, size)
  ...
  private def applyDCT(f: Array[Array[Double]]): Array[Array[Double]] = {
    val F: Array[Array[Double]] = Array.ofDim[Double](size, size)
    for (u <- 0 until size) {
      for (v <- 0 until size) {
        var sum: Double = 0.0
        for (i <- 0 until size) {
